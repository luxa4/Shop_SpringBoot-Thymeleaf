...
;$(function(){
	...
	var loadMoreMyOrders = function (){
		...
		convertButtonToLoader(btn, 'btn-success');
		var pageNumber = parseInt($('#myOrders').attr('data-pages-number'));
		var url = '/ajax/html/more/my-orders?pages=' + (pageNumber + 1);
		$.ajax({
			url : url,
			success : function(html) {
				$('#myOrders tbody').append(html);
				pageNumber++;
				var pageCount = parseInt($('#myOrders').attr('data-pages-count'));
				$('#myOrders').attr('data-pages-number', pageNumber);
				if (pageNumber < pageCount) {
	...
	/*var loadMore = function (btn, urlTemplate, attrContainer, appendContainer, onSuccess){
		convertButtonToLoader(btn, 'btn-success');
		var pageNumber = parseInt(attrContainer.attr('data-pages-number'));
		...
		$.ajax({
			...
			success : function(html) {
				...
				pageNumber++;
				var pageCount = parseInt(attrContainer.attr('data-pages-count'));
				attrContainer.attr('data-pages-number', pageNumber);
				if (pageNumber < pageCount) {
	...
	var loadMoreProducts = function (){
		loadMore(
			$('#loadMore'), 
			'/ajax/html/more' + location.pathname + '?pages=$PAGE_NUM&' + location.search.substring(1),
			$('#productList'),
			$('#productList .row'),
			function(){
				initBuyBtn();
	...
	var loadMoreMyOrders = function (){
		loadMore(
			$('#loadMoreMyOrders'), 
			'/ajax/html/more/my-orders?pages=$PAGE_NUM',
			$('#myOrders'),
			$('#myOrders tbody')
		);
	};*/
